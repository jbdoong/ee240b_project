...
def design_inv_tia(specs, pch_op_info, nch_op_info, pch_scale):
    ...
    voff = specs['voff']
    ber_targ = specs['ber']
    snr_like_targ = np.sqrt(2) * sp.special.erfcinv(2 * ber_targ)
    ...
    for scale in scale_vec:
        for rf in np.linspace(rf_max, rf_min, n_rf):
            # Resistor parasitic capacitances
            # Cpar_in
            # Cpar_l
            ...
            if f3db >= f3db_targ:
                ...
                if snr_like >= snr_like_targ:
                    ...
                    if pm >= pm_targ:
                        ...
                        power = ibias * vdd
                        ber = 1/2 * sp.special.erfc(snr_like / np.sqrt(2))
                        design = OrderedDict(
                            Scale = scale,
                            Rf = rf,
                            Power = power,
...
def run_main():
    ...
    specs = dict(
        ...
        # vgs_res = 5e-3,
        vdd = 1.2,
        cin = 20e-15, # 20, 20, 100
        cl = 10e-15, # Cgg: ?, 8.84e-15, ?
        isw = 1e-6,
        tper = 200e-12, # 120, 200, 200
        voff = 20e-3, # ?
        ber = 1e-12,
        f_factor = 2/3,
        pm = 45,
